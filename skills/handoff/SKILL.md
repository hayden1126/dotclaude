...
2. **Reconcile the docs (the gate).** Dispatch a read-only sub-agent (`Explore`) to compare the change set against the durable non-CLAUDE docs (SPEC.md, README.md, design/architecture docs, anything describing behavior you changed) and return the stale claims. Fix where the correction is unambiguous; flag what needs a human decision. Never silently rewrite. For CLAUDE.md specifically, delegate to claude-md-management (`/revise-claude-md` to propose additions with approval, `claude-md-improver` to audit), do not hand-edit it here. The handoff is not complete until the durable docs match the code.
